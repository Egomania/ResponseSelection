...
class Solver(SolverBase):
    ...
    def create_problem(self, data): 
        ...
        for entry in responses_used:
            for hostElem in entry.dest:
                ...
                listEntry = [host, response, costGes, cost, len(entry.dest)]
    ...
    def solve_problem(self, problem, config):
        ...
        for name, value in problem[1].iteritems():
	...
	adminList.sort(key=lambda x: (x[0]))
        adminList.sort(key=lambda x: (x[2]))
        adminList.sort(key=lambda x: (x[4]), reverse=True)
